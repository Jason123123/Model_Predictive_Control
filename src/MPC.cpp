...
// index of state variables and actuators
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  const int W_EPSI = 100;
  const int W_V = 1;
  const int W_DELTA = 10000;
  const int W_A = 1 ;
  const int W_DDELTA = 100;
  const int W_DA = 100;
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      //calculate f'
      AD<double> f_prime = 0.0;
      for(int i = 0; i < coeffs.size() - 1; i++){
        f_prime += (i + 1) * coeffs[i + 1] * CppAD::pow(x0, i);
      }
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 - v0 * delta0 / Lf * dt);//invert delta
